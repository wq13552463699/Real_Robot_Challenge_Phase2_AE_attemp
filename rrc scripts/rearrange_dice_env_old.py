...
class SimtoRealRearrangeDiceEnv(gym.GoalEnv):
    ...
    def _create_observation(self, t, action):
        ...
        # print(camera_observation.cameras)
        ...
        observation = {
            ...
            "action": action,
            "desired_goal": np.array(self.goal_masks),
            "achieved_goal": np.array(segmentation_masks),
        }
        ...
        # print(des_goal)
        # print(observation["robot_observation"]["position"].shape)
        # print(observation["robot_observation"]["velocity"].shape)
        # print(observation["robot_observation"]["torque"].shape)
        # print(observation["action"].shape)
        # print(observation["desired_goal"].shape)
        # print(observation["achieved_goal"].shape)
        # image = observation["desired_goal"]
        # image = image.transpose(1, 2, 0)
        # image = cv2.resize(image, (64,64), interpolation=cv2.INTER_LINEAR)
        # (B, G, R) = cv2.split(image)
        # image2 = observation["achieved_goal"]
        # image2 = image2.transpose(1, 2, 0)
        # image2 = cv2.resize(image, (64,64), interpolation=cv2.INTER_LINEAR)
        # (BB, GG, RR) = cv2.split(image2)
        # cv2.imwrite("B.jpg", B)
        # cv2.imwrite("G.jpg", G)
        # cv2.imwrite("R.JPG", R)
        # cv2.imwrite("BB.jpg", BB)
        # cv2.imwrite("GG.jpg", GG)
        # cv2.imwrite("RR.JPG", RR)
        # cv2.imwrite("image2222.jpg", image2)
        # cv2.imwrite("image1111.jpg", image)
    ...
    def step(self, action):
        ...
        pos = observation["robot_observation"]["position"]
        # vel = observation["robot_observation"]["velocity"]
        # tor = observation["robot_observation"]["torque"]
        act = observation["action"]
        
        des_goal = copy.copy(self.goal)
        des_goal = np.array(des_goal)
        des_goal = des_goal.reshape(75,)
        
        ach_goal = copy.copy(observation["achieved_goal"])
        ach_goal = np.array(ach_goal)
        ach_goal = ach_goal.transpose(1, 2, 0)
        ach_goal = cv2.resize(ach_goal, (64,64), interpolation=cv2.INTER_LINEAR)
        ach_goal = ach_goal.reshape(12288,)
        
        obs = np.concatenate((pos,act,des_goal,ach_goal), axis=0)
        
        
        return obs, reward, is_done, self.info
